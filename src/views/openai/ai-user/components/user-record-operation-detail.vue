...
<template>
  ...
  >
    ...
    >
      ...
      <div style="margin: 10px 0" v-if="!data.params || !data.result">
        <a-alert
          message="如出现'请求参数'或'返回结果'无内容，有可能因为数据太大被忽略。属正常现象。"
          show-icon
          banner
        />
      </div>
      ...
      >
        <template v-if="!data.result">
          <span
            v-if="
              data.contentType &&
              data.contentType.startsWith('text/event-stream')
            "
            style="color: #f37575; font-size: 12px"
          >
            此类型数据不记录返回结果
          </span>
          <span v-else></span>
...
<script lang="ts" setup>
  ...
  import TextEllipsis from '@/views/system/operation-record/components/text-ellipsis.vue';
