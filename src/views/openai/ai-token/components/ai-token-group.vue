    ...
    >
      ...
      <div>
        <ele-pro-table
          ...
          :height="400"
          :where="defaultWhere"
          cache-key="proOpenaiTokenGroupTable"
          row-key="id"
        >
          <template #toolbar>
            <a-space>
...
<script lang="ts" setup>
  ...
  import {
    ...
    removeAiTokenGroups,
    updateAiTokenGroup
  } from '@/api/openai/ai-token-group';
  import type {
    AiTokenGroup,
    AiTokenGroupParam
  } from '@/api/openai/ai-token-group/model';
  import {
    ColumnItem,
    DatasourceFunction
  ...
  // 表单数据
  const { form, resetFields, assignFields } = useFormData<AiTokenGroup>({
    id: undefined,
    name: '',
    code: ''
  });
  ...
  // 表格选中数据
  const selection = ref<AiTokenGroup[]>([]);
  ...
  const datasource: DatasourceFunction = ({ page, limit, where, orders }) => {
    return pageAiTokenGroups({ ...where, ...orders, page, limit });
  };
  ...
  /* 打开编辑弹窗 */
  const openEdit = (row?: AiTokenGroup) => {
    if (row) {
      assignFields({
        ...row
      });
  ...
  /* 删除单个 */
  const remove = (row: AiTokenGroup) => {
    const hide = messageLoading('请求中..', 0);
    removeAiTokenGroup(row.id)
      .then((msg) => {
        hide();
        message.success(msg);
        reload();
  ...
  const removeBatch = () => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        const hide = messageLoading('请求中..', 0);
        removeAiTokenGroups(selection.value.map((d) => d.id))
          .then((msg) => {
            hide();
            message.success(msg);
            reload();
  ...
  /* 搜索 */
  const reload = (where?: AiTokenGroupParam) => {
    selection.value = [];
