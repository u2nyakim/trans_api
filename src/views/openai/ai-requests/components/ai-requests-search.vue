    ...
    <a-row :gutter="8">
      ...
      >
        <a-form-item label="账单uuid">
          <a-input
            v-model:value.trim="form.billUuid"
            placeholder="请输入"
            allow-clear
          />
        </a-form-item>
      ...
      >
        <a-form-item label="请求状态码">
          <a-input
            v-model:value.trim="form.statusCode"
            placeholder="请输入"
            allow-clear
          />
        </a-form-item>
...
import type { AiRequestsParam } from '@/api/openai/ai-requests/model';
...
const emit = defineEmits<{
  (e: 'search', where?: AiRequestsParam): void;
}>();

// 表单数据
const { form, resetFields } = useFormData<AiRequestsParam>({
  billUuid: '',
  statusCode:'',
});
