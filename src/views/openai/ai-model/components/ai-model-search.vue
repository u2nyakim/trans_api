    ...
    <a-row :gutter="8">
      ...
      >
        <a-form-item label="收费方式">
          <a-select
            v-model:value="form.feeType"
            placeholder="请选择"
            allow-clear
          >
            <a-select-option :value="0">免费</a-select-option>
            <a-select-option :value="1">按次数计费</a-select-option>
            <a-select-option :value="2">按Token计费</a-select-option>
          </a-select>
        </a-form-item>
      </a-col>
      <a-col
...
<script lang="ts" setup>
  ...
  import type { AiModelParam } from '@/api/openai/ai-model/model';
  ...
  const emit = defineEmits<{
    (e: 'search', where?: AiModelParam): void;
  }>();

  // 表单数据
  const { form, resetFields } = useFormData<AiModelParam>({
    name: '',
    status: undefined,
    feeType: undefined,
    ...props.where
